            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                <div className="p-6 rounded-lg border" style={{ background: 'var(--card-bg)', borderColor: 'rgba(255,255,255,0.1)' }}>
                  ...
                  <p className="text-xs text-gray-600 mt-2">Workers: {stats.totalWorkers} | Providers: {stats.totalProviders}</p>
                </div>
                <div className="p-6 rounded-lg border" style={{ background: 'var(--card-bg)', borderColor: 'rgba(255,255,255,0.1)' }}>
                  <p className="text-sm text-gray-600 mb-2">Total Jobs</p>
                  <p className="text-3xl font-bold text-black">{stats.totalJobs}</p>
                  <p className="text-xs text-gray-600 mt-2">Open: {stats.openJobs} | Completed: {stats.completedJobs}</p>
                </div>
                <div className="p-6 rounded-lg border" style={{ background: 'var(--card-bg)', borderColor: 'rgba(255,255,255,0.1)' }}>
                  <p className="text-sm text-gray-600 mb-2">Total Earnings</p>
                  <p className="text-3xl font-bold text-black">₹{stats.totalEarnings.toLocaleString()}</p>
                  <p className="text-xs text-gray-600 mt-2">Transactions: {stats.totalTransactions}</p>
                </div>
                <div className="p-6 rounded-lg border" style={{ background: 'var(--card-bg)', borderColor: 'rgba(255,255,255,0.1)' }}>
                  <p className="text-sm text-gray-600 mb-2">Pending Reports</p>
                  <p className="text-3xl font-bold text-black">{stats.pendingReports}</p>
